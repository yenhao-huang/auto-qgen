...
請只回覆 JSON 格式的結果，不要包含其他說明文字。"""
    ...
    def transform_to_augmenterInput(
        ...
        """
        將 Excel Parser LLM 的輸出轉換為 AugmenterInput 格式
        ...
        # 提取基本資訊
        file_name = excel_parser_output.get("file_name", "")
        ...
        for cell in cells_to_process:
            x_axis_id = cell.get("x-axis id", "")
            y_axis_id = cell.get("y-axis id", "")
            ...
            # 構建 ground_truth
            # 注意：GroundTruth 的 item_id 需要是 List[int]
            # 這裡我們將 y-axis id 轉換為整數（如果可能）
            try:
                y_axis_id_int = int(y_axis_id)
                item_id = [y_axis_id_int]
            ...
            # 構建 metadata（包含原始的 x-axis id 和 y-axis id）
            metadata = {
                "x-axis id": x_axis_id,
                "y-axis id": y_axis_id,
